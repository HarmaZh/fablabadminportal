...
const ROLE_BADGE: Record<string, string> = {
  ...
  'YMCA Coordinator': 'bg-blue-100 text-blue-700',
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-4xl font-bold text-jet-black">Staff & Instructors</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        <div className="card flex items-center gap-5 hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-xs font-semibold text-primary-500 uppercase tracking-wide mb-1">Total Staff</p>
            <p className="text-3xl font-bold text-jet-black leading-none">{staff.length}</p>
          </div>
        </div>
        ...
        <div className="card flex items-center gap-5 hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-xs font-semibold text-green-600 uppercase tracking-wide mb-1">Active Instructors</p>
            <p className="text-3xl font-bold text-jet-black leading-none">{activeInstructors}</p>
          </div>
        </div>
        ...
        <div className="card flex items-center gap-5 hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-xs font-semibold text-blue-600 uppercase tracking-wide mb-1">YMCA Outreach</p>
            <p className="text-3xl font-bold text-jet-black leading-none">{ymcaCount}</p>
          </div>
        </div>
        ...
        <div className="card flex items-center gap-5 hover:shadow-md transition-shadow">
          ...
          <div>
            <p className="text-xs font-semibold text-yellow-600 uppercase tracking-wide mb-1">Volunteers</p>
            <p className="text-3xl font-bold text-jet-black leading-none">{volunteers}</p>
          </div>
        </div>
      </div>
      ...
      <div className="card mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-jet-black mb-2">Role</label>
      ...
      <div className="card overflow-x-auto">
        {filtered.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-primary-600 text-lg mb-4">No staff found</p>
          ...
          <table className="min-w-full divide-y divide-pale-sky">
            <thead className="bg-gradient-to-r from-pale-sky/30 to-light-blue/30">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-bold text-jet-black uppercase tracking-wider">Staff ID</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-jet-black uppercase tracking-wider">Name</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-jet-black uppercase tracking-wider">Role</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-jet-black uppercase tracking-wider">Email</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-jet-black uppercase tracking-wider">Specialties</th>
                <th className="px-6 py-4 text-left text-xs font-bold text-jet-black uppercase tracking-wider">School</th>
            ...
            <tbody className="bg-white divide-y divide-pale-sky/50">
              {filtered.map((member) => (
                <tr key={member.id} className="hover:bg-pale-sky/10 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-3 py-1 text-xs font-bold rounded-full ${ROLE_BADGE[member.role] ?? 'bg-gray-100 text-gray-600'}`}>
                      {member.role}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {member.specialties.slice(0, 2).map((s) => (
                        <span key={s} className="bg-pale-sky/40 text-jet-black text-xs px-2 py-0.5 rounded-full whitespace-nowrap">
                          {s}
                        </span>
                      ))}
                      {member.specialties.length > 2 && (
                        <span className="bg-pale-sky/20 text-primary-600 text-xs px-2 py-0.5 rounded-full whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    {member.school ? (
                      <span className="text-jet-black font-medium">{member.school}</span>
                    ) : (
      ...
      >
        <form className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-jet-black mb-2">First Name *</label>
              <input type="text" className="input" placeholder="Maria" defaultValue={selectedStaff?.firstName} />
            </div>
            <div>
              <label className="block text-sm font-semibold text-jet-black mb-2">Last Name *</label>
              <input type="text" className="input" placeholder="Garcia" defaultValue={selectedStaff?.lastName} />
            </div>
          </div>

          <div>
            <label className="block text-sm font-semibold text-jet-black mb-2">Email *</label>
            <input type="email" className="input" placeholder="staff@fablab.org" defaultValue={selectedStaff?.email} />
          </div>

          <div>
            <label className="block text-sm font-semibold text-jet-black mb-2">Phone</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-jet-black mb-2">Specialties</label>
            <input
              type="text"
              className="input"
            ...
            />
            <p className="mt-1 text-xs text-primary-600">Separate multiple specialties with commas.</p>
          </div>

          <div>
            <label className="block text-sm font-semibold text-jet-black mb-2">Outreach School</label>
            <input
              type="text"
              className="input"
            ...
            />
            <p className="mt-1 text-xs text-primary-600">For YMCA Coordinators — leave blank if not applicable.</p>
          </div>
